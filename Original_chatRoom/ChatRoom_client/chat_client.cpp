...
class chat_client{
    ...
    void do_read_body(){                                                        //异步读取TCP连接中的数据体
        ...
        [this](boost::system::error_code ec,std::size_t /*length*/){
            if(!ec){
                if(read_msgs_.body_length()==sizeof(RoomInformation)&& read_msgs_.type()==MT_SERVER_INFO){
                const RoomInformation *info =reinterpret_cast<const RoomInformation *>(read_msgs_.body());
                std::cout<<"client: ";
                assert(info->name.nameLen<=sizeof(info->name.name));
                std::cout.write(info->name.name,info->name.nameLen);
                std::cout<<" 发送：";
                std::cout<<info->chat.infoLen;
